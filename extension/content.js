...
// Extract Crunchyroll show info from page.
function detectCrunchyrollShowInfo() {
  let showTitle = '';
  ...
  return { showTitle: showTitle || '', episodeInfo };
}
...
function detectNetflixShowInfo() {
  ...
  if (titleEl) {
    ...
    if (firstChild) {
      ...
      if (t && t.toLowerCase() !== 'netflix') showTitle = t;
    }
    if (!episodeInfo) {
...
function detectShowInfo() {
  ...
  const base = { platform, url: location.href, detectedAt: new Date().toISOString() };

  if (platform === 'crunchyroll') {
    const { showTitle, episodeInfo } = detectCrunchyrollShowInfo();
